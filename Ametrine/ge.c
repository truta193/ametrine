...
bool VBOSetIdentifier(VBO *vbo, char *iden);
...
bool VAOSetIdentifier(VAO *vao, char *iden);
...
bool ImageSetIdentifier(ImageT *image, char *iden);
...
bool TextureSetIdentifier(TextureT *texture, char *iden);
...
bool CompareIdentifier(char *identifier, char *string);
...
void EventHandler() {
    ...
    while (XPending(display)) {
        ...
        switch(event.type) {
            case Expose: {
                XWindowAttributes windowAttam_engine *am_engine_instance;
...
bool VBOSetIdentifier(VBO *vbo, char *iden) {
    if (strlen(iden) > 16) return false;
    strcpy(vbo->identifier, iden);
    return true;
};
...
return_code EventUserInit() {
    ...
    ArrayPush(&VAOs, &vao, 1);
    ArrayPush(&VBOs, &vbo, 1);
    ArrayPush(&VBOs, &idx, 1);
...
bool TextureSetIdentifier(TextureT *texture, char *iden) {
    if (strlen(iden) > 16) return false;
    strcpy(texture->identifier, iden);
    return true;
};
...
bool CompareIdentifier(char *identifier, char *string) {
    if (strcmp(identifier, string) == 0) return true;
    return false;
};
...
void FrameRender() {
    ...
    VAOBind(&arrayRetrieve(&VAOs, VAO, 0));
